...
BANVAR = 35
BORW = 59
Blacklist = [[:UNKNOWNBEAM, :DOOMESTDESIRE, :PRESENTSIGHT, :ENERGYROCKET, :THUNDERSTRIKE, :GLITCH, :SKYBUG], [], [90]]
...
class MoveRelearnerScreen
    ...
    def pbGetRelearnableMoves(pkmn)
        ...
        if $game_variables[MOVETUTOR] == 6 # hackmon
            hmoves = hackmoves
            for i in hmoves
                moves.push(i) if !pkmn.hasMove?(i) && !moves.include?(i) && validmove(i)
            end
        ...
        if $game_variables[MOVETUTOR] == 7 # universal move tutor
            for i in UCmoves
                moves.push(i) if !pkmn.hasMove?(i) && !moves.include?(i) && validmove(i)
            end
        end
        if $game_variables[MOVETUTOR] == 8 # custom move tutor
            pmoves = [:JUDGMENT]
