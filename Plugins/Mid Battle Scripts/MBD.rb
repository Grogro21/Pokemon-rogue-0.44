...
class PokemonTemp
  ...
  def excludedialogue
	@excludedialogue=[] if !@excludedialogue
    return @excludedialogue
  end
end
...
# HP Reduction Dialogue
  def pbReduceHP(amt,anim=true,registerDamage=true,anyAnim=true)
    ...
    if self.hp < (self.totalhp*0.25).floor && (!self.damageState.lowHP || self.effects[PBEffects::Lowhp] == true) && self.hp>0 && !$PokemonTemp.excludedialogue.include?(self.index)
    ...
    elsif self.hp < (self.totalhp*0.5).floor && self.hp > (self.totalhp*0.25).floor && !self.damageState.halfHP && !$PokemonTemp.excludedialogue.include?(self.index)
      self.damageState.halfHP = true
	  self.damageState.highHP = true
      if !opposes?
	...
	elsif self.hp < (self.totalhp*0.75).floor && self.hp > (self.totalhp*0.5).floor && !self.damageState.highHP && !$PokemonTemp.excludedialogue.include?(self.index)
      self.damageState.highHP = true
      if !opposes?
...
# Big,Small and Low, Mid HP Dialogue Dialogue
  def pbEndureKOMessage(target)
    ...
    if target.hp <= (target.totalhp*0.25).floor && target.hp>0 && !$PokemonTemp.excludedialogue.include?(target.index)
    ...
    elsif target.hp <= (target.totalhp*0.5).floor && target.hp>0 && !$PokemonTemp.excludedialogue.include?(target.index)
	...
	elsif target.hp <= (target.totalhp*0.75).floor && target.hp>0 && !$PokemonTemp.excludedialogue.include?(target.index)
...
module TrainerDialogue
  ...
  def self.setInstance(parameter)
    noIncrement = ["lowHP","lowHPOpp","halfHP","halfHPOpp","highHP","highHPOpp","bigDamage","bigDamageOpp","smlDamage",
      "smlDamageOpp","attack","attackOpp","superEff","superEffOpp","notEff","notEffOpp","fainted","faintedOpp"]
